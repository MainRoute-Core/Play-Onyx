...
const settings = {
    ...
    reset: async () => {
        if (confirm("FACTORY RESET: Delete everything and reload?")) {
            try {
                await app.db.wipe();
                localStorage.removeItem("PlayOnyx_Settings");
                location.reload();
            } catch (e) {
                indexedDB.deleteDatabase("PlayOnyx");
                location.reload();
            }
        }
    },
...
const app = {
    ...
    init: async () => {
        ...
        if (cfg.queue && cfg.queue.length > 0) {
            queue.list = cfg.queue
                .map((qid) => app.items.find((i) => i.id === qid))
                .filter((item) => item);
            queue.render();
        }
        ...
        if (cfg.lastID) {
            const lastItem = app.items.find((i) => i.id === cfg.lastID);
            if (lastItem) {
                await player.load(lastItem, true, cfg.lastTime || 0);

                if (cfg.activeTab === "playing") app.nav("playing");
                console.log(`Restoring: ${lastItem.name} at ${cfg.lastTime}s`);
            }
        }
        ...
        if (dz) {
            ...
            dz.ondrop = async (e) => {
                ...
                if (e.dataTransfer.files.length)
                    await app.processFiles(e.dataTransfer.files);
            };
        }
    },

    clearDB: async () => {
        if (confirm("Permanently delete ALL media files?")) {
            try {
                await app.db.wipe();
                app.items = [];
                store.state.queue = [];
    ...
    nav: (tab) => {
        ...
        if (tab === "playing") {
            ...
            let title = "Library";
            if (tab === "audio") {
                list = app.items.filter((i) => i.type.startsWith("audio"));
                title = "Audio";
            }
            if (tab === "video") {
                list = app.items.filter((i) => i.type.startsWith("video"));
                title = "Video";
            }
            if (tab === "favorites") {
                list = app.items.filter((i) => i.fav);
                title = "Favorites";
            }
    ...
    sortLib: (method) => {
        store.state.sortBy = method;
        store.save();
        app.applySort(app.items);
        app.nav(store.state.activeTab);
    },

    applySort: (list) => {
        const m = store.state.sortBy;
        list.sort((a, b) => {
            if (m === "name") return a.name.localeCompare(b.name);
            if (m === "size") return b.size - a.size;
    ...
    refreshLib: async () => {
        let all = await app.db.getAll();
        app.items = app.applySort(all);
        $("#lib-count").innerText = app.items.length;

        $("#sort-select").value = store.state.sortBy || "date";

        if (store.state.activeTab !== "playing")
            app.nav(store.state.activeTab);
    },
    ...
    playOrFocus: (item) => {
        if (player.active && player.active.id === item.id) {
            ...
            if (!queue.list.some((i) => i.id === item.id)) queue.add(item);
    ...
    openModal: (id) => {
        ...
        if (el) {
            ...
            if (id === "meta-modal") {
                if (!player.active) {
                    $("#meta-body").innerHTML = "No Active File";
                    return;
                }
...
const audio = {
    ...
    init: () => {
        try {
            ...
            audio.bands.low.type = "lowshelf";
            audio.bands.low.frequency.value = 320;
            audio.bands.mid.type = "peaking";
            audio.bands.mid.frequency.value = 1000;
            audio.bands.high.type = "highshelf";
            audio.bands.high.frequency.value = 3200;

            const st = store.state.eq;
            audio.setEq("low", st.low);
            audio.setEq("mid", st.mid);
            audio.setEq("high", st.high);
    ...
    loopVis: () => {
        ...
        const waveCvs = document.getElementById("wave-vis");
        if (!cvs || !waveCvs) return;
        ...
        const draw = () => {
            ...
            if (waveCvs.width !== waveCvs.clientWidth) {
            ...
            if (!player.active || player.el.paused || !audio.analyser) return;
            ...
            const timeBuf = new Uint8Array(audio.analyser.fftSize);
            audio.analyser.getByteTimeDomainData(timeBuf);

            waveCtx.lineWidth = 2;
            waveCtx.strokeStyle = "#d0bcff";
            waveCtx.beginPath();

            const sliceW = (waveCvs.width * 1.0) / audio.analyser.fftSize;
            let xVal = 0;

            for (let i = 0; i < audio.analyser.fftSize; i++) {
                const v = timeBuf[i] / 128.0;
                const yVal = (v * waveCvs.height) / 2;
                if (i === 0) waveCtx.moveTo(xVal, yVal);
                else waveCtx.lineTo(xVal, yVal);
                xVal += sliceW;
            }
            waveCtx.stroke();
        };
...
const player = {
    ...
    shuffle: false,
    loop: false,
    load: async (item, paused = false, startTime = 0) => {
        if (!item || !item.blob) {
            app.toast("Error: File not found");
            return;
        }
        ...
        const onMeta = () => {
            if (startTime > 0) {
                player.el.currentTime = startTime;
            } else {
                player.el.currentTime = 0;
            }
            store.state.lastID = item.id;
            store.save();

            player.el.removeEventListener("loadedmetadata", onMeta);
        };
        player.el.addEventListener("loadedmetadata", onMeta);
        ...
        ui.renderCtrls();
        queue.render();
        $("#view-player").style.background = "#000";

        if (item.thumb) {
            const img = new Image();
            img.src = item.thumb;
            img.onload = () => ui.setBg(img);
        } else if (item.type.startsWith("video")) {
            const onData = () => {
                ui.setBg(player.el);
                player.el.removeEventListener("loadeddata", onData);
            };
            player.el.addEventListener("loadeddata", onData);
        }
    },
...
player.el.onended = () =>
    player.loop ? player.el.play() : player.next();
...
player.el.ontimeupdate = () => {
    ...
    if (player.active) {
        const now = Date.now();
        if (!player._lastSave || now - player._lastSave > 1000) {
            store.state.lastID = player.active.id;
    ...
    genThumb: (item) => {
        ...
        vid.onloadeddata = () => {
            setTimeout(() => {
                ...
                app.db.update(item);
                if (
                    store.state.activeTab === "library" ||
                    store.state.activeTab === "video"
                )
                    app.refreshLib();
                if (player.active && player.active.id === item.id)
                    $("#curr-thumb").style.backgroundImage = `url(${item.thumb})`;
            }, 500);
        };
    },
    setBg: (sourceElement) => {
        try {
            ...
            ctx.drawImage(sourceElement, 0, 0, 1, 1);

            const [r, g, b] = ctx.getImageData(0, 0, 1, 1).data;

            const color = `rgb(${r}, ${g}, ${b})`;
            $("#view-player").style.background =
                `radial-gradient(circle at center, ${color} 0%, #050505 85%)`;
        } catch (e) {
            $("#view-player").style.background = "#000";
        }
    },
